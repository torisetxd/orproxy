...
function getApiKey(req) {
	...
	if (!key && h["x-openrouter-api-key"])
		key = String(h["x-openrouter-api-key"]).trim();
...
function setRLHeaders(res, entry) {
	...
	res.setHeader(
		...
		String(Math.max(0, RL_PER_MINUTE - entry.mCount))
	);
	res.setHeader(
		"x-ratelimit-reset-minute",
		String(Math.ceil(entry.mReset / 1000))
	);
	res.setHeader("x-ratelimit-limit-30m", String(RL_PER_30MIN));
	res.setHeader(
		"x-ratelimit-remaining-30m",
		String(Math.max(0, RL_PER_30MIN - entry.hCount))
	);
	res.setHeader(
		"x-ratelimit-reset-30m",
		String(Math.ceil(entry.hReset / 1000))
	);
}
...
function applyRateLimit(req, res) {
	...
	if (!allowed) {
		...
		const retryMs = Math.max(
			0,
			Math.min(entry.mReset - now, entry.hReset - now)
		);
		res.writeHead(429, {
